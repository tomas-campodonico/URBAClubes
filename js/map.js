...
function Club(name, dir, lat, lon, waypoints) {
	this.nombre = name;
	this.direccion = dir;
	this.latitud = lat;
	this.longitud = lon;
...
initializeClubes = function(){
	var club,
		option;

	clubes = {};
	clubes['Albatros'] = new Club('Albatros Rugby Club', 'Calle 135 y 515 - La Plata', -34.91309, -58.016962);
	...
	clubes['Manuel Belgrano - Sede Maschwitz'] = new Club('Club Manuel Belgrano Maschwitz', 'La Pista 1601, esq. Lago Puelo - Maschwitz', -34.363577, -58.736561);
	...
	clubes['San Luis'] = new Club('Club San Luis', 'Calle 520 e/ 27 y 29 - La Plata', -34.911059, -58.002899);
	//clubes[''] = new Club('', '', );
	//clubes[''] = new Club('', '', );
...
$(document).ready( function() {
	...
	bsas = new google.maps.LatLng(-34.840013, -58.265991);
	mapOptions = {
		zoom:9,
	...
	document.getElementById("select-club").addEventListener("change", calcRoute, false);  
});
...
calcRoute = function() {
	...
	start = new google.maps.LatLng(clubes["San Luis"].latitud, clubes["San Luis"].longitud),
  	endClub = document.getElementById("select-club").value;

	if (typeof clubes[endClub] !== 'undefined') {
		end = new google.maps.LatLng(clubes[endClub].latitud, clubes[endClub].longitud);
		request = {
			origin: start,
			destination: end,
